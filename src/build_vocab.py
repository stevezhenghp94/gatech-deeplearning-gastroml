...
def build_vocab_recipe1m(args):
    ...
    data = json.load(open(os.path.join(args.recipe1m_path, 'final_data.json'), 'r'))
    ...
    if os.path.exists(ingrs_file) and os.path.exists(instrs_file) and not args.forcegen:
        ...
        for i, entry in tqdm(enumerate(data)):
            ...
            det_ingrs = eval(entry['ingredients_rawmats']) # convert string-represented list to list

            #valid = data[idx2ind[entry['id']]]['valid']
            det_ingrs_filtered = []
    ...
    for i, entry in tqdm(enumerate(data)):
        ...
        if entry['id'] in id2im.keys():
            images_list.append(entry['image_name']) # only one image per recipe                
